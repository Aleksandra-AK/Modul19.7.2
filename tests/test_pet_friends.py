...
def test_post_add_new_pet_with_valid_data(name='Куропатыч', animal_type='петух',
...
def test_delete_pet_correct():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "Суперкотттт", "кот", "3", "images/1174945.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
...
def test_update_pet_info_successful(name='Куропа', animal_type='голубь', age='5'):
...
def test_create_pet_simple_without_photo_valid(name='jgfd', animal_type='hgd', age='4'):
...
def test_add_photo(pet_photo='images/maxresdefault.jpg'):
    ...
    if len(my_pets['pets']) > 0:
        for pet in my_pets['pets']:
            ...
            if pet['pet_photo'] == '':
                status, result = pf.add_photo_of_a_pet(auth_key, pet['id'], pet_photo)
                pet_id = pet['id']
...
def test_post_add_new_pet_with_invalid_data_name(name='', animal_type='Spider',
...
def test_post_add_new_pet_with_png_photo_file(name='Мурзик', animal_type='Кот',
